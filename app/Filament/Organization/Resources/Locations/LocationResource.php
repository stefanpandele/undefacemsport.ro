...
class LocationResource extends Resource
{
    ...
    protected static ?string $model = OrganizationLocation::class;
    ...
    protected static function locationFields(): array
    {
        return [
            ...
            TextInput::make('address')
                ...
                ->rule(static function ($get, ?OrganizationLocation $record, ?Component $livewire): Closure {
                    return static function (string $attribute, mixed $value, Closure $fail) use ($get, $record, $livewire): void {
                        ...
                        if ($organization instanceof Organization && $organization->organizationLocationAt($get('county'), $get('city'), $value, $record?->getKey()) !== null) {
                ...
                ->rule(static function ($get, ?OrganizationLocation $record): Closure {
                ...
                ->suffixAction(
                    Action::make('geocode')
                        ...
                        ->action(function ($get, $set, Component $livewire, ?OrganizationLocation $record): void {
                            $missing = collect([
        ...
        $set('nearby_candidates', $candidates->isEmpty() ? null : $candidates
            ->mapWithKeys(fn (Location $location): array => [
                (string) $location->getKey() => $location->name.' — la '.round((float) $location->distance_meters).' m'
                    .' ('.trans_choice('{0} niciun club|{1} un club|[2,*] :count cluburi', $location->organizationLocations()->count()).')',
            ])
            ->all());
    }
     ...
     */
    protected static function unresolvedNeighbours(callable $get, ?OrganizationLocation $record): Collection
    {
     ...
     */
    protected static function isAlreadyMine(Location $location, ?Component $livewire, ?OrganizationLocation $record): bool
    {
        $organization = static::resolveOrganization($livewire);
            ...
            Select::make('new_facilities')
                ...
                ->createOptionForm([
                    TextInput::make('name')
                        ...
                        // defaults it on, and the record here is the OrganizationLocation
                        // being edited — which would filter `facilities` by
                        // `organization_location.id`. This modal only ever creates.
                ...
                ->createOptionUsing(function (array $data, $get, $set, ?Component $livewire): int {
                    $facilityId = static::createSuggestedFacility($data, static::resolveOrganization($livewire));
     ...
     */
    public static function createSuggestedFacility(array $data, ?Organization $organization): int
    {
        ...
        return Action::make('proposeCorrection')
            ...
            ->schema([
                Placeholder::make('current')
                    ...
                    ->content(fn (OrganizationLocation $record): string => implode(' · ', array_filter([
     ...
     *
     * @param  array<string, mixed>  $data
     * @param  Organization|null  $organization  the owning club when there is no panel context to read it from
     */
    public static function persist(
        array $data,
        ?OrganizationLocation $record = null,
        ?Component $livewire = null,
        ?Organization $organization = null,
    ): OrganizationLocation {
        $organization ??= $record instanceof OrganizationLocation ? $record->organization : static::resolveOrganization($livewire);
     ...
     */
    public static function fillFromRecord(array $data, OrganizationLocation $record): array
    {
        $location = $record->location;
    ...
    protected static function existingFacilitiesLabel(mixed $locationId): string
    {
        ...
        $names = Facility::constrainUsable(
            Facility::query(),
            $tenant instanceof Organization ? $tenant : null,
        )
